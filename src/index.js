...
export default class PrismaZoom extends PureComponent {
  ...
  getNewPosition = (x, y, zoom) => {
    const [prevZoom, prevPosX, prevPosY] = [
      this.state.zoom,
      this.state.posX,
      this.state.posY,
    ];
    ...
    if (zoom > prevZoom) {
      ...
      const [absX, absY] = [
        (centerX - relativeX) / prevZoom,
        (centerY - relativeY) / prevZoom,
      ];
  ...
  startDeceleration = (lastShiftOnX, lastShiftOnY) => {
    ...
    const move = timestamp => {
      ...
      const ratio =
        (this.props.decelerationDuration - progress) /
        this.props.decelerationDuration;
    ...
    if (!this.props.disableScroll) {
      ...
      if (zoom !== this.state.zoom) {
        if (zoom !== minZoom) {
          ...
          [posX, posY] = [
            this.constructor.defaultState.posX,
            this.constructor.defaultState.posY,
          ];
        }
      }
  ...
  handleTouchMove = event => {
    ...
    if (event.touches.length === 1) {
      ...
      const distance = Math.sqrt(
        Math.pow(pos2X - pos1X, 2) + Math.pow(pos2Y - pos1Y, 2),
      );

      if (
        this.lastTouchDistance &&
        distance &&
        distance !== this.lastTouchDistance
      ) {
